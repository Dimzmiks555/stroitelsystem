...
export class RealisationsService {
  ...
  async findAll(): Promise<any> {
    const url = encodeURI(
      'http://server/1CBase/odata/standard.odata/Document_РеализацияТоваров?$orderby=Date desc',
    );
  ...
  async findOne(id: string) {
    const url = encodeURI(
      `http://server/1CBase/odata/standard.odata/Document_РеализацияТоваров?$filter=Ref_Key eq guid'${id}'`,
    );
    const url2 = encodeURI(
      `http://server/1CBase/odata/standard.odata/Document_РеализацияТоваров_Товары?$filter=Ref_Key eq guid'${id}'`,
    );
